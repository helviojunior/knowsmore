...
from knowsmore.libs.ntdsuseraccount import NTDSUserAccount
...
class NTDSHashes:
        ...
        if haveInfo is True:
            ...
            for propertyCount in range(userProperties['PropertyCount']):
                ...
                if userProperty['PropertyName'].decode('utf-16le') == 'Primary:Kerberos-Newer-Keys':
                    ...
                    try:
                        pwd = "%s" % unhexlify(userProperty['PropertyValue']).decode('utf-16le')
    ...
    def __decryptHash(self, record, prefixTable=None, outputFile=None):
        ...
        if self.__useVSSMethod is True:
            full_name = record[self.NAME_TO_INTERNAL['name']]
            ...
            answer = NTDSUserAccount(
                ...
                user_name=userName,
                rid=rid,
                object_sid=sid.formatCanonical(),
                lm_ash=hexlify(LMHash).decode('utf-8'),
                nt_hash=hexlify(NTHash).decode('utf-8'),
            ...
            replyVersion = 'V%d' %record['pdwOutVersion']
            full_name = record['pmsgOut'][replyVersion]['pNC']['StringName'][:-1]
            ...
            if self.__history:
                for i, (LMHashHistory, NTHashHistory) in enumerate(
                    ...
                    answer = NTDSUserAccount(
                        ...
                        object_sid=objectSid,
                        lm_ash=lmhash.decode('utf-8'),
                        nt_hash=hexlify(NTHashHistory).decode('utf-8'),
                        pwd_last_set='',
                        status='',
                        history=i+1
                    )
